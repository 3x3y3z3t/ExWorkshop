...
namespace exw
{
    namespace utils
    {
        ref<spdlog::logger> Logger::s_Core_logger;
        ...
        void Logger::init()
        {
            ...
            sinks.emplace_back(exw::create_ref<spdlog::sinks::stdout_color_sink_mt>());
            sinks.emplace_back(exw::create_ref<spdlog::sinks::basic_file_sink_mt>(buffer, true)); // truncate = true;

            sinks[0]->set_pattern("%^[%C.%m.%d %H:%M:%S.%e][%t][%n] %v%$");
            sinks[1]->set_pattern("[%C.%m.%d %H:%M:%S.%e][%t][%n][%-8l] %v");

            s_Core_logger = create_ref<spdlog::logger>("CORE", std::begin(sinks), std::end(sinks));
            spdlog::register_logger(s_Core_logger);
            s_Core_logger->set_level(spdlog::level::trace);
            s_Core_logger->flush_on(spdlog::level::trace);

            s_Client_logger = create_ref<spdlog::logger>("APP ", std::begin(sinks), std::end(sinks));
